...
/* Ã¢â€â‚¬Ã¢â€â‚¬ SUPABASE CLIENT (CDN) Ã¢â€â‚¬Ã¢â€â‚¬ */
...
async function sendEmail(type, data) {
  try {
    ...
    // Non-blocking Ã¢â‚¬â€ don't prevent form submission if email fails
...
function loadAnalytics() {
  ...
  // Microsoft Clarity (Heatmap Ã¢â‚¬â€ Free)
...
/* -- WINDOW.SVSITE -- expose shared API so inline page scripts can call window.SVSite.insertRecord() etc. */
window.SVSite = {
  config: SV_CONFIG,
  db: db,
  openModal: openModal,
  ...
  uploadFile: async function(bucket, file, folder) {
    folder = folder || SV_CONFIG.confId;
    var ext = file.name.split('.').pop();
    var safeName = folder + '/' + Date.now() + '_' + Math.random().toString(36).slice(2,8) + '.' + ext;
    var result = await db.storage.from(bucket).upload(safeName, file, { cacheControl: '3600', upsert: false });
    if (result.error) throw result.error;
    var urlResult = db.storage.from(bucket).getPublicUrl(result.data.path);
