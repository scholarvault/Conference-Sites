...
(function () {
  ...
  function renderCard(person, type) {
    ...
    var linkedinHTML = '';
    if (linkedin) {
      linkedinHTML = '<a href="' + esc(linkedin) + '" target="_blank" rel="noopener" aria-label="LinkedIn Profile" class="person-card__linkedin" title="LinkedIn Profile">' + linkedinSVG + '</a>';
    }
    ...
    if (type === 'speaker') {
      ...
      shareHTML = '<button aria-label="Share speaker profile" class="person-card__share" onclick="shareSpeaker(\'' + rawName + '\',\'' + rawType + '\',\'' + rawTopic + '\')" title="Share">' + shareSVG + '</button>';
    }
