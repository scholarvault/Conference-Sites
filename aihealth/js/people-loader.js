...
(function () {
  ...
  function renderCard(person, type) {
    ...
    var shareHTML = '';
    if (type === 'speaker') {
      // SENTINEL: Escape backslashes and single quotes for JS literal, then escape for HTML attribute
      var rawName = esc((person.name || '').replace(/\\/g, '\\\\').replace(/'/g, "\\'"));
      var rawType = esc((person.talk_type || '').replace(/\\/g, '\\\\').replace(/'/g, "\\'"));
      var rawTopic = esc((person.topic || '').replace(/\\/g, '\\\\').replace(/'/g, "\\'"));
