...
## 2026-10-15 - Layout Thrashing with getBoundingClientRect on scroll
**Learning:** Invoking `getBoundingClientRect()` repeatedly inside a `requestAnimationFrame` render loop on `scroll` forces synchronous layout recalculation, leading to severe layout thrashing. Because `scroll` shifts elements dynamically, their rect.top values become skewed, making continuous updates both expensive and visually inaccurate for calculated effects like parallax offsets.
**Action:** Always map initial positions relative to `window.scrollY` (e.g. `rect.top + window.scrollY + rect.height / 2`) and cache these as absolute coordinates. During the `scroll` rAF loop, avoid DOM reads like `getBoundingClientRect()` entirely and calculate transforms purely mathematically using the current `window.scrollY` against the cached layout. Use a debounced or ticked `resize` listener to refresh the cache.
