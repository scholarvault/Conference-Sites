...
## 2026-04-29 - DOM XSS via Unescaped JS Strings in HTML Attributes
**Vulnerability:** User-provided inputs (e.g., speaker names) were HTML-escaped and then directly injected into JavaScript string literals within HTML event attributes (e.g., `onclick="shareSpeaker('...', '...', '...')"`). An attacker could input a name like `\'); alert(1); //` which `escapeHtml` leaves untouched except for escaping the single quote to `&#039;`. However, the browser's HTML parser decodes `&#039;` back to `'` before the JS engine executes the attribute, resulting in the script breaking out of the string literal and executing `alert(1)`.
**Learning:** HTML escaping is not sufficient when injecting dynamic data into JavaScript string literals within HTML attributes. The HTML parser decodes entities prior to JavaScript execution, meaning characters like single quotes, double quotes, and backslashes will regain their literal syntactic meaning in JS context.
**Prevention:** Always escape JS-context-breaking characters (`\`, `'`, `"`, `\n`, `\r`) with backslashes first, before applying HTML escaping. For example: `escapeHtml(escapeJs(input))`.

## 2026-05-15 - XSS via Unescaped JSON-LD Schema Generation
**Vulnerability:** The function generating the JSON-LD schema (`generatePersonSchema`) in `aihealth/js/people-loader.js` serialized the JSON object using `JSON.stringify` and immediately injected the output string into the HTML DOM within a `<script type="application/ld+json">` tag via `innerHTML`. Since JSON serialization preserves `<` characters verbatim, an attacker could supply input containing `</script><script>alert(1)</script>`, which would prematurely close the schema tag and execute the malicious script on the page.
**Learning:** `JSON.stringify` is not intrinsically safe for injection into an HTML context. When placing stringified JSON into an HTML `<script>` block, any unescaped `</script>` literal within the JSON values will be interpreted by the HTML parser as a closing tag, leading to a stored XSS vulnerability.
**Prevention:** Always escape the `<` character in stringified JSON payloads before injecting them into HTML script blocks. A standard approach is replacing `<` with its Unicode equivalent: `JSON.stringify(data).replace(/</g, '\\u003c')`. This retains structural JSON validity while neutralizing the HTML parser threat.
