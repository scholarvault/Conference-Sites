...
## 2026-04-28 - Reflected XSS via HTML Attribute Breakout in JavaScript Literal
**Vulnerability:** User-provided inputs (`name`, `talk_type`, `topic`) in `aihealth/js/people-loader.js` were injected into a JavaScript string literal inside an HTML `onclick` attribute. The implementation attempted to escape single quotes via `replace(/'/g, "\\'")`, but failed to escape backslashes and HTML entities. An attacker could break out of the JS string and the HTML attribute (e.g. using `hello" autofocus onfocus="alert(1)`) to execute arbitrary JavaScript, leading to Cross-Site Scripting (XSS).
**Learning:** When injecting dynamic data into JS string literals that are themselves within HTML attributes, a single quote escape is insufficient. The string must first be escaped for the JavaScript context (both backslashes `\` and single quotes `'`), and then the entire resulting literal must be HTML-escaped (e.g., using `esc()`) to prevent breaking out of the surrounding HTML attribute.
**Prevention:** Always escape for both contexts consecutively. First escape `\` to `\\` and `'` to `\'` to safely construct the JS literal, then pass the result through an HTML entity escape function to safely embed the string into the HTML attribute.
