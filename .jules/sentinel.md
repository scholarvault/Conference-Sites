...
## 2026-05-13 - Prevent DOM XSS via Escaped JavaScript inside HTML attributes
**Vulnerability:** User-controlled values passed into 'onclick' attributes were only HTML escaped (e.g. `&#039;`). HTML entities decode prior to JS execution. So input like `\');alert(1);//` resulted in JS execution (DOM XSS breakout).
**Learning:** When embedding user input into JS string literals inside HTML attributes, it is critical to first escape JS context-breaking characters (like backslashes and quotes) with backslashes before applying HTML escaping. Just HTML escaping is insufficient.
**Prevention:** Always double escape the input for attributes acting as JS handlers using a pattern like `escapeHtml(escapeJs(input))`.
