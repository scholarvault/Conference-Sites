/**
 * SVRIDS 2027 - Academic Enrichment & ROR Autocomplete Engine
 */
(function () {
  'use strict';
  ...
  function initRorAutocomplete() {
    ...
    inputs.forEach((input) => {
      ...
      dropdown.style.boxShadow = '0 12px 30px rgba(0,0,0,0.35)';
          ...
          try {
            ...
            dropdown.innerHTML = currentResults
              .map((it, idx) => {
                ...
                return `
                  <li data-index="${idx}" role="option" style="padding: 10px 14px; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,0.06); font-size: 14px; color: #f8fafc;">
                    <div style="font-weight: 600;">${escapeHtml(name)}</div>
                    ${loc ? `<div style="font-size: 12px; color: #94a3b8;">${escapeHtml(loc)}</div>` : ''}
                  </li>
                `;
              })
              .join('');
