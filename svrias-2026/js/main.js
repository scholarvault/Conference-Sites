...
function initSecureConferenceJourneys() {
  const slug = "research-integrity-responsible-ai-summit-2026";
  const appOrigin = getScholarVaultAppOrigin();
  const loginUrl = (action) => `${appOrigin}/login?next=${encodeURIComponent(`/dashboard/conferences/${slug}/${action}`)}`;
